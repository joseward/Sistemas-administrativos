          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 relative z-10 flex flex-col items-center">
            ...
            {!selectedRole ? (
              ...
              <div className="space-y-1.5">
                ...
                {/* Botón de Submit */}
                <Link href={selectedRole === 'admin' ? '/' : '/portal-docente'} className="block w-full pt-2">
                  <button type="button" className={`w-full text-white font-medium py-3 rounded-xl transition-colors shadow-md hover:shadow-lg ${selectedRole === 'admin' ? 'bg-[#1877f2] hover:bg-[#1564cc]' : 'bg-emerald-600 hover:bg-emerald-700'}`}>
                    Iniciar Sesión
                  </button>
                </Link>
              </form>
